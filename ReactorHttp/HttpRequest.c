...
{
	char* end = bufferFindCRLF(readBuf);   //一行信息
	if (end != NULL)
	{
		char* start = readBuf->data + readBuf->readPos;
		int lineSize = end - start;
    ...
    struct HttpResponse* response, struct Buffer* sendBuf, int socket)
{
    ...
    while (request->curState != ParseReqDone)
    {
        ...
        {
            /*
                HTTP响应数据的四个部分： 1.状态行 2.响应头 3.空行 4.响应体
            */

            // processHttpRequest： 
            // 1. 根据解析出的原始数据, 对客户端的请求做出处理（实际上就是根据请求行的url
            //    判断资源类型（目录还是文件），然后将响应的状态行、响应头键值对数据存在response结构体中，
            //    以及初始化response里面的发送响应体数据的函数指针（就那个回调函数））。
...
// 处理基于get的http请求，将一些数据存在response结构体里面
...
#ifndef MSG_SEND_AUTO
            bufferSendData(sendBuf, cfd);  //响应数据
#endif
        }
        else if (len == 0)
        {
